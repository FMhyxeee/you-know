...
export default function HomePage() {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).__TAURI__) {
        ...
        setArticles(articlesData.map(article => ({
          ...article,
          feedName: feedsData.find(f => f.id === article.feed_id)?.title || 'Unknown Feed',
          readTime: '5 min read',
        ...
        setError('Running in browser mode - RSS feeds are only available in the desktop app')
        setFeeds([])
        setArticles([])
      }
    } catch (err) {
  ...
  const handleAddFeed = async (url: string) => {
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).__TAURI__) {
        ...
        setArticles(articlesData.map(article => ({
          ...article,
          feedName: [...feeds, newFeed].find(f => f.id === article.feed_id)?.title || 'Unknown Feed',
          readTime: '5 min read',
          summary: article.description || article.content?.substring(0, 200) + '...' || 'No summary available'
        })))
        return true
  ...
  const handleUpdateArticle = async (articleId: string, updates: { is_read?: boolean; is_starred?: boolean }) => {
    try {
      if (typeof window !== 'undefined' && (window as any).__TAURI__) {
  ...
  const handleRefreshFeed = async (feedId: string) => {
    try {
      ...
      const enrichedArticles = articlesData.map(article => ({
        ...article,
        feedName: feeds.find(f => f.id === article.feed_id)?.title || 'Unknown',
        readTime: '5 min read',
  ...
  const handleDeleteFeed = async (feedId: string, feedTitle: string) => {
    if (confirm(`Are you sure you want to delete "${feedTitle}"?`)) {
      try {
        ...
        const enrichedArticles = articlesData.map(article => ({
          ...article,
          feedName: feedsData.find(f => f.id === article.feed_id)?.title || 'Unknown',
          readTime: '5 min read',
          summary: article.content?.substring(0, 200) + '...' || ''
        }))
        setArticles(enrichedArticles)
